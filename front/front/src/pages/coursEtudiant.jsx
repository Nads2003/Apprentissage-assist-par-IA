...
useEffect(() => {
  ...
  return (
    <div className="container mx-auto p-6 pt-[80px]">
      ...
      {/* --- HEADER MODERNE --- */}
      <motion.div
        className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4 bg-black/10 backdrop-blur-lg p-6 rounded-2xl shadow-lg"
      ...
      >
        ...
        {/* Barre de recherche */}
        <div className="flex items-center bg-white backdrop-blur-md rounded-xl p-2 w-full md:w-1/3">
          <Search size={20} className="text-gray/70 mr-2" />
          <input
            type="text"
            placeholder="Rechercher un cours..."
            className="bg-transparent focus:outline-none text-gray placeholder-gray/50 w-full"
            value={search}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCours.map((c) => (
          ...
          >
              ...
              <button
                ...
                className={`absolute top-3 right-3 p-2 rounded-full transition-transform ${
                  favoris.includes(c.id)
                    ...
                    : "bg-gray-100 text-gray-500 hover:text-red-500 hover:scale-110"
                }`}
                title={
                  favoris.includes(c.id)
                    ? "Retirer des favoris"
            ...
            {/* Card Content */}
            <div className="p-5 flex bg-slate-900 rounded-2xl flex-col justify-between h-[240px]">
...
">
  {c.description || "Aucune description fournie."}
              ...
              <div className="flex flex-wrap gap-2 mb-1 items-center">
                <div className="flex items-center gap-2 bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full text-xs font-bold">
                  <img
                ...
                <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium hover:bg-green-200 transition">
                  🏛️ {c.mention?.nom}
                </span>

                <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-full text-xs font-medium hover:bg-purple-200 transition">
                  🎓 {c.niveau?.nom}
                </span>
              </div>
              ...
              <button
                ...
                className="mt-2 w-full bg-indigo-600 text-white py-2 rounded-xl hover:bg-indigo-700 transition text-sm font-semibold flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
              >
                📄 Voir le PDF
              </button>
              <button
